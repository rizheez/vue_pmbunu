...
import type { PmbUser, Fakultas, RegistrationPath, RegistrationPeriod, RegistrationType } from '@/types/pmb';
...
const form = ref({
    ...
    // Registration
    period_id: props.student.registration?.registration_period_id ? String(props.student.registration.registration_period_id) : '',
    type_id: props.student.registration?.registration_type_id ? String(props.student.registration.registration_type_id) : '',
    path_id: props.student.registration?.registration_path_id ? String(props.student.registration.registration_path_id) : '',
    program_studi_1: props.student.registration?.choice_1 ? String(props.student.registration.choice_1) : '',
    program_studi_2: props.student.registration?.choice_2 ? String(props.student.registration.choice_2) : '',
    // Referral
...
const breadcrumbs = [
    ...
    { title: 'Edit Mahasiswa', href: `/admin/students/${props.student.id}/edit` },
];
</script>
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">Perbarui data calon mahasiswa</p>
                    </div>
                </div>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Upload ulang jika ingin mengubah file (maks 2MB per file)</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center gap-2">
                                    <label class="flex cursor-pointer items-center gap-2 rounded-md border border-dashed px-4 py-2 hover:bg-gray-50">
                                        <Upload class="size-4" />
                                        <span class="text-sm">{{ files.photo?.name || 'Ganti file...' }}</span>
                                        <input type="file" class="hidden" accept="image/*" @change="handleFileChange('photo', $event)" />
                                    </label>
                                    <span v-if="!files.photo && props.student.student_biodata?.photo_path" class="text-xs text-green-600">File tersimpan</span>
                                </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center gap-2">
                                    <label class="flex cursor-pointer items-center gap-2 rounded-md border border-dashed px-4 py-2 hover:bg-gray-50">
                                        <Upload class="size-4" />
                                        <span class="text-sm">{{ files.ktp?.name || 'Ganti file...' }}</span>
                                        <input type="file" class="hidden" accept="image/*,.pdf" @change="handleFileChange('ktp', $event)" />
                                    </label>
                                    <span v-if="!files.ktp && props.student.student_biodata?.ktp_path" class="text-xs text-green-600">File tersimpan</span>
                                </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center gap-2">
                                    <label class="flex cursor-pointer items-center gap-2 rounded-md border border-dashed px-4 py-2 hover:bg-gray-50">
                                        <Upload class="size-4" />
                                        <span class="text-sm">{{ files.kk?.name || 'Ganti file...' }}</span>
                                        <input type="file" class="hidden" accept="image/*,.pdf" @change="handleFileChange('kk', $event)" />
                                    </label>
                                    <span v-if="!files.kk && props.student.student_biodata?.kk_path" class="text-xs text-green-600">File tersimpan</span>
                                </div>
                                <p class="text-xs text-gray-500">Maks 2MB (PDF/JPG/PNG)</p>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center gap-2">
                                    <label class="flex cursor-pointer items-center gap-2 rounded-md border border-dashed px-4 py-2 hover:bg-gray-50">
                                        <Upload class="size-4" />
                                        <span class="text-sm">{{ files.certificate?.name || 'Ganti file...' }}</span>
                                        <input type="file" class="hidden" accept="image/*,.pdf" @change="handleFileChange('certificate', $event)" />
                                    </label>
                                    <span v-if="!files.certificate && props.student.student_biodata?.certificate_path" class="text-xs text-green-600">File tersimpan</span>
                                </div>
                            </div>
                        </div>
                    </CardContent>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div class="grid gap-4 sm:grid-cols-2">
                             <div class="space-y-2">
                                <Label>Periode Pendaftaran <span class="text-red-500">*</span></Label>
                                <Select v-model="form.period_id">
                                    <SelectTrigger class="w-full">
                                        <SelectValue placeholder="Pilih periode" />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem v-for="p in props.periods" :key="p.id" :value="String(p.id)">
                                            {{ p.name }} <template v-if="p.is_active">(Aktif)</template>
                                        </SelectItem>
                                    </SelectContent>
                                </Select>
                                <p v-if="errors.period_id" class="text-sm text-red-500">{{ errors.period_id }}</p>
                            </div>
                            <div class="space-y-2">
                ...
                <div class="flex justify-end gap-3">
                    <Button type="button" variant="outline" as-child>
                        <Link :href="`/admin/students/${props.student.id}`">Batal</Link>
                    </Button>
