...
const editing = ref<RegistrationPath | null>(null);
...
const openEdit = (item: RegistrationPath) => {
    editing.value = item;
    form.name = item.name;
...
const submit = () => {
    if (editing.value) {
        form.put(`/admin/registration-paths/${editing.value.id}`, {
            onSuccess: () => (showDialog.value = false),
        });
    } else {
        form.post('/admin/registration-paths', {
            onSuccess: () => (showDialog.value = false),
        });
    }
};
...
const deleteItem = (id: number) => {
    if (confirm('Hapus jalur pendaftaran ini?')) {
        router.delete(`/admin/registration-paths/${id}`);
    }
};

const breadcrumbs = [
    { title: 'Admin Dashboard', href: '/admin/dashboard' },
    { title: 'Jalur Pendaftaran', href: '/admin/registration-paths' },
];
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
                    ...
                    <div class="overflow-x-auto rounded-lg border">
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y">
                                <tr v-for="item in props.paths" :key="item.id" class="hover:bg-gray-50">
        ...
        <Dialog v-model:open="showDialog">
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>{{ editing ? 'Edit' : 'Tambah' }} Jalur Pendaftaran</DialogTitle>
                </DialogHeader>
                <form @submit.prevent="submit" class="space-y-4">
                    <div class="space-y-2">
                        <Label>Nama</Label>
                        <Input v-model="form.name" placeholder="Jalur Reguler" />
                    </div>
                    <div class="space-y-2">
